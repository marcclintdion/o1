#include    "colorDepthShadows_END.cpp"//_AVOID_MEMORY_LEAKS_AT_RUNTIME._THIS_DEACTIVATES_EXISTING_BUFFERS_BEFORE_RE-LOADING.
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=
...
filePathName = [[NSBundle mainBundle] pathForResource:@"colorDepthShadows" ofType:@"vert"];
vertexSource_colorDepthShadows   = readShaderFile([filePathName cStringUsingEncoding:NSASCIIStringEncoding]);
filePathName = [[NSBundle mainBundle] pathForResource:@"colorDepthShadows" ofType:@"frag"];
fragmentSource_colorDepthShadows = readShaderFile([filePathName cStringUsingEncoding:NSASCIIStringEncoding]);
//===============
